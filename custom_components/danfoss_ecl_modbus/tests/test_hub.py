...
async def test_update_success(mock_client):
    ...
    mock_client.read_holding_registers = AsyncMock(return_value=success_response)
    mock_client.read_input_registers = AsyncMock(return_value=success_response)
...
async def test_write_register(mock_client):
    ...
    mock_client.write_register.assert_called_with(REG_HVAC_MODE, 1, slave=5)
